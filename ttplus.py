...
# Add a grey placeholder for a new task
# @return newly created task id
# @note always keep track of newly created task id otherwise impossible to save later
...
def update_task_name(event=None):
    ...
    if selected_item:
        ...
        if values[const.TABLE_FIELD_TASK_NAME] != new_name: ## the name could be the same if a control key pressed on keyboard
